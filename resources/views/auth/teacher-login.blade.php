...
<div class="flex fle-col items-center justify-center p-6">
    <div class="grid lg:grid-cols-2 items-center gap-8 max-w-6xl max-lg:max-w-lg w-full">
        <form class="lg:max-w-md w-full" action="{{ route('teacher.login') }}" method="post">
            @csrf
            <h1 class="text-slate-900 text-3xl font-semibold mb-8">Login Guru</h1>
            <div class="space-y-6">
                <div>
                    <label class="text-slate-900 text-sm mb-2 block">NIK</label>
                    @if ($errors->any())
                        <input name="nik" type="number" maxlength="16" class="bg-gray-100 w-full text-slate-900 text-sm px-4 py-3 focus:bg-transparent border border-red-500 focus:border-black outline-none transition-all" placeholder="Masukkan NIK" value="{{ old('nik') }}" required/>
                    ...
                    @else
                        ...
                        @else
                            <input name="nik" type="number" maxlength="16" class="bg-gray-100 w-full text-slate-900 text-sm px-4 py-3 focus:bg-transparent border border-grey-100 focus:border-black outline-none transition-all" placeholder="Masukkan NIK" value="{{ old('nik') }}" required/>
